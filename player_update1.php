...
//
    ...
    <table class="w3-table-all w3-container">  
		...
		while (($row = oci_fetch_row($query))) { ?>
            <form action="player_update2.php" method="post">
            ...
            <td><input type="hidden" name="playername" value="<?php echo $row[0]; ?>">
                <input type="hidden" name="playerage" value="<?php echo $row[1]; ?>">
                <input type="hidden" name="playerclub" value="<?php echo $row[2]; ?>">
                <input type="hidden" name="playerposition" value="<?php echo $row[3]; ?>">
                <input type="hidden" name="playerrating" value="<?php echo $row[4]; ?>">
                <input type="hidden" name="playervalue" value="<?php echo $row[5]; ?>">
                <input type="hidden" name="playerinjury" value="<?php echo $row[6]; ?>">
