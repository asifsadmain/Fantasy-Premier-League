...
<p class="w3-center w3-xxlarge">List of Left Backs</p>
...
where P_POS='LB' and user_id=(SELECT USER_ID FROM USERLIST WHERE USERNAME='$username')";
...
if($count==0){
    $show = "SELECT PLAYER_NAME,AGE,CLUB,POS,RATING,TRANSFER_VALUE,INJURY FROM PLAYERS WHERE POS='LB'";
}
else
{
    $show = "SELECT PLAYER_NAME,AGE,CLUB,POS,RATING,TRANSFER_VALUE,INJURY FROM PLAYERS WHERE POS='LB' and player_id<>'$player_id'";
}
...
while (($row = oci_fetch_row($query))) {
    ...
    <form action="profile2.php" method="post">
        <input type="hidden" name="hidden_lb" value="<?php  echo $row[0]; ?>">    
        <input type="submit" class="w3-button w3-hover-blue" name="addLB" value="Add">
    </form>
    <?php
    echo "</td>";
    echo "</tr>\n";
