...
where P_POS='RS' and user_id=(SELECT USER_ID FROM USERLIST WHERE USERNAME='$username')";
...
while (($row = oci_fetch_row($query))) {
    ...
    <form action="profile2.php" method="post">
        <input type="hidden" name="hidden_rs" value="<?php  echo $row[0]; ?>">  
        <input type="submit" class='w3-button w3-hover-blue' name="addRS" value="Add">
    </form>
    <?php
    echo "</td>";
    echo "</tr>\n";
